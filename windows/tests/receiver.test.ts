...
test('HTTPS pairing, authenticated upload, deduplication, validation, restart and revocation', async () => {
  ...
  try {
    ...
    const writing = await import('sharp').then(({ default: sharp }) => sharp({ create: { width: 900, height: 1200, channels: 4, background: '#ffffff' } }).png().toBuffer());
    const writingUpload = await call(receiver, 'PUT', `/photos/${randomUUID()}`, writing, photoHeaders(credential, writing, 'image/png'));
    assert.equal(writingUpload.status, 201); assert.ok(receiver.photoPath(writingUpload.body.id)!.endsWith('.png')); assert.deepEqual(await readFile(receiver.photoPath(writingUpload.body.id)!), writing);
    const writingThumb = await receiver.thumbnail(writingUpload.body.id); assert.ok(writingThumb); assert.equal((await (await import('sharp')).default(writingThumb).metadata()).format, 'jpeg');
    assert.equal((await call(receiver, 'PUT', `/photos/${randomUUID()}`, image, { ...headers, 'Content-Type': 'image/gif' })).status, 415);
    for (let i = 0; i < 19; i++) assert.equal((await call(receiver, 'PUT', `/photos/${randomUUID()}`, image, headers)).status, 201);
    assert.equal(receiver.photos().length, 21);
    ...
    assert.equal(receiver.fingerprint, fingerprint); assert.equal(receiver.port, rememberedPort); assert.equal(receiver.photos().length, 22); assert.equal(receiver.directory, nextFolder);
